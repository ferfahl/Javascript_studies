...
function checkForWinner() {
    ...
    if (p1_points === 5) {
      gameover = true;
      ctx.fillText("Player 1 wins!", w/4, h/2);
      console.log("Player 1 wins!");
      // Add any additional logic for ending the game
    } else if (p2_points === 5) {
      gameover = true;
      ctx.fillText("Player 2 wins!", w/4, h/2);
      console.log("Player 2 wins!");
      // Add any additional logic for ending the game
    }
    if (gameover)
      ctx.fillText("Press R to restart the game", w/4, 3*h/4);
  }
...
document.addEventListener("keydown",function(ev) {
    if (gameover) {
        ...
        if (ev.keyCode == 82) {
            resetGame();
            draw();  // Redraw the game after resetting
        }
        return;
    }
